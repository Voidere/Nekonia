...
@onready var sounds = {
	...
	&"backbutton" : AudioStreamPlayer.new(),
	}
...
func _ready() -> void:
	...
	for i in sounds.keys():
		sounds[i].stream = load("res://assets/sounds/sfx/" + str(i) + ".mp3")
...
func install_sounds(node: Node) -> void:
	for i in node.get_children():
		if i is Button:
			i.mouse_entered.connect( ui_sfx_play.bind(&"button") )
			i.pressed.connect( ui_sfx_play.bind(&"backbutton") )
			...
			i.tab_hovered.connect( ui_sfx_play.bind(&"button") )
			i.tab_clicked.connect( ui_sfx_play.bind(&"backbutton") )
		install_sounds(i)
