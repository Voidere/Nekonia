...
func complete_level():
	...
	if level_number < LevelManager.unlocked_levels.size() - 1:  # Check if it's not the last level
		LevelManager.unlock_level(level_number + 3)  # Unlock the next level
	print("Level ", level_number, " completed. Unlocking Level ", level_number + 3)
...
#	LevelManager.check_winter_level_unlock()
